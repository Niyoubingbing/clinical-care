...
## 6. 历史仓库恢复说明（2026-08-17）

2026-09-06 已在基于 origin/main 的正常 Git 分支上工作，之前放弃的界面重构已保存在本地 stash，当前通过 PR 发布 2.18.0。以下历史情况不代表当前状态。
...
本次审查已从本地 remote URL 移除嵌入的 GitHub 令牌；令牌撤销/轮换仍需仓库所有者在 GitHub 完成。系统中的 Vercel 令牌在 2026-08-17 审查时认证失败；2026-09-06 已验证当前环境凭据可读取项目及部署。凭据值不得写入文档或日志。
