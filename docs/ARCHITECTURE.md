...
> 状态：当前规范  
> 核对日期：2026-09-06
> 实现基线：应用 `2.18.0`（基于 2.17.3；发布状态以 GitHub/Vercel 为准）
...
- 领域判断的唯一来源应位于 `lib/`；尤其床型统一通过 `recognizeBed`（单床修正优先，未识别不隐藏），查房顺序统一通过 `resolveOrder`。
...
5. `version.json` 使用网络直读/no-store，仅用于远程版本比较；当前运行版本来自构建包。离线 RSC .txt 根据不含查询参数的固定键匹配，避免与 HTML 混用。
...
床位识别与迁移的详细规则见 [2.18.0 设计与发布说明](./RELEASE-2.18.0.md)。
