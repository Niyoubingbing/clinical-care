  ...
  event.waitUntil(
    caches
      ...
      .then((keys) =>
        ...
        Promise.all(
          keys
            .filter((k) => k.startsWith("clinical-care-v") && k !== CACHE && k !== PATIENT_SHELL_KEY)
...
async function handleNavigate(request, url) {
  const cached = await caches.match(url.pathname === "/" ? "/" : url.pathname.replace(/\/$/, ""), { cacheName: CACHE });
  ...
  try {
    ...
    if (isPatientRoute(url.pathname)) {
      const shell = await caches.match("/patient", { cacheName: CACHE }) || await caches.match(PATIENT_SHELL_KEY, { cacheName: CACHE });
...
// Exported RSC payloads are static by pathname; _rsc query hashes must not fragment offline keys.
// HTML and RSC always use different keys.
function assetKey(request, url) {
  if (url.pathname.endsWith(".txt")) return url.pathname;
  if (request.headers.get("RSC") === "1") return (url.pathname === "/" ? "/index" : url.pathname.replace(/\/$/, "")) + ".txt";
  return request;
}
...
async function handleAsset(request, url) {
  ...
  try {
    ...
    if (res && res.status === 200 && (res.type === "basic" || res.type === "cors")) {
      ...
      if (isPatientRoute(url.pathname) && res.headers.get("content-type")?.includes("text/html")) {
        await cachePatientShell(cache, res);
      }
    }
    return res;
